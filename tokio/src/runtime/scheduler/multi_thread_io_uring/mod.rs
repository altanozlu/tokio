...
pub(crate) struct MultiThreadUring;

// ===== impl MultiThread =====

impl MultiThreadUring {
    pub(crate) fn new(
        size: usize,
        driver: Driver,
        ...
        config: Config,
    ) -> (MultiThreadUring, Arc<Handle>, Launch) {
        ...
        (MultiThreadUring, handle, launch)
    }
    ...
    pub(crate) fn shutdown(&mut self, handle: &scheduler::Handle) {
        match handle {
            scheduler::Handle::MultiThreadUring(handle) => handle.shutdown(),
            _ => panic!("expected MultiThread scheduler"),
        }
    }
}

impl fmt::Debug for MultiThreadUring {
