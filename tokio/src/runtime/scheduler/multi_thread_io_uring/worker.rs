...
use crate::runtime::scheduler::multi_thread_io_uring::{
...
pub(crate) struct Context {
    ...
    // driver: RefCell<Option<crate::runtime::io::uring::Handle>>,
}
...
fn run(worker: Arc<Worker>) {
    ...
    let handle = scheduler::Handle::MultiThreadUring(worker.handle.clone());
    println!("Setting worker {:?}", std::thread::current().id());
    crate::runtime::context::enter_runtime(&handle, true, |_| {
        // URING_CONTEXT.with(|uc| {
        // Set the worker context.
        let cx = scheduler::Context::MultiThreadUring(Context {
            worker,
            core: RefCell::new(None),
            defer: Defer::new(),
            // driver: uc.driver.clone(),
        });

        context::set_scheduler(&cx, || {
            let cx = cx.expect_multi_thread_uring();
...
impl Context {
    fn run(&self, mut core: Box<Core>) -> RunResult {
        ...
        println!("Run worker {:?}", std::thread::current().id());
        while !core.is_shutdown {
            ...
            // {
            //     for fd in core.uring_sqes {
            //         // let ac = opcode::Accept::new(
            //         //     types::Fd(fd),
            ...
            //         //     .build();
            //         // URING_CONTEXT.with(|uc| {
            //         //     uc.handle().submit_op(await?)
            //         // })
            //         let op = Op::accept(&SharedFd::new(fd)).unwrap();
            //     }
            // }
            ...
            if let Some(task) = core.steal_work(&self.worker) {
                ...
                // Wait for work
                // {
                //     for ring in self.worker.handle.rings.lock().iter() {
                //        let mut afd = AsyncFd::new(ring.inner.get_mut().as_raw_fd()).unwrap();
                //     }
                // }
    ...
    fn maintenance(&self, mut core: Box<Core>) -> Box<Core> {
        // println!("maintenance {:?}", std::thread::current().id());
    ...
    fn park(&self, mut core: Box<Core>) -> Box<Core> {
        ...
        //     // self.worker.handle.driver.io().add_source()
        //     // let mut afd = AsyncFd::with_interest(uc.handle().inner.borrow().as_raw_fd(), Interest::READABLE).unwrap();
        //     //     uc.handle().wait();
        //     //     uc.handle().flush().expect("TODO: panic message");
        // });
...
fn with_current<R>(f: impl FnOnce(Option<&Context>) -> R) -> R {
    use scheduler::Context::{MultiThreadUring};

    context::with_scheduler(|ctx| match ctx {
        Some(MultiThreadUring(ctx)) => f(Some(ctx)),
        _ => f(None),
    })
}
